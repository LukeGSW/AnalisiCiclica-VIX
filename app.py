...
def load_data_from_path(ticker_path):
    ...
    try:
        ...
        # ================================================================= #
        #               <<< CORREZIONE DEFINITIVA CARICAMENTO DATI >>>      #
        # ================================================================= #
        # 1. Forza la conversione dell'indice a DatetimeIndex
        df.index = pd.to_datetime(df.index, utc=True)
        # 2. Rimuovi qualsiasi informazione sul fuso orario per standardizzare
...
def create_equity_chart(df_results: pd.DataFrame):
    """Create the equity curve chart from backtest results."""
    fig = go.Figure()
    if 'equity' in df_results.columns:
        fig.add_trace(go.Scatter(x=df_results.index, y=df_results['equity'], name='Strategy', line=dict(color='blue', width=2)))
    if 'benchmark_equity' in df_results.columns:
        fig.add_trace(go.Scatter(x=df_results.index, y=df_results['benchmark_equity'], name='Buy & Hold', line=dict(color='gray', width=1, dash='dash')))
    fig.update_layout(title='Equity Curve Comparison', xaxis_title='Date', yaxis_title='Portfolio Value ($)', height=400, hovermode='x unified', template='plotly_white')
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        **Trading Fees:** {float(Config.TRADING_FEES)*100:.1f}%
        ...
        new_lookback = st.slider("Lookback period (years)", 1, 20, value=current_lookback, step=1, help="Number of years of historical data to analyze")
        ...
        if 'cycle_analysis' in summary:
            ...
            if cycle_info.get('dominant_period'):
                st.metric("Dominant Cycle", f"{float(cycle_info['dominant_period']):.1f} days")
            
            st.metric("Statistical Significance", f"p-value: {float(cycle_info.get('p_value', 1)):.4f}", "✅ Significant" if cycle_info.get('significant', False) else "⚠️ Not Significant")
        ...
        # ================================================================= #
        # Poiché i dati sono ora standardizzati (timezone-naive), il confronto è diretto e sicuro.
        if len(date_range) == 2:
    ...
    with tab2:
        ...
        if 'phase_quadrant' in df_filtered.columns:
            ...
            with col1:
                ...
                fig_pie = go.Figure(data=[go.Pie(labels=phase_counts.index.tolist(), values=phase_counts.values.tolist(), hole=0.3)])
                fig_pie.update_layout(title="Time Spent in Each Phase", height=300)
                st.plotly_chart(fig_pie, use_container_width=True)
            with col2:
                total_signals = len(df_filtered[df_filtered['signal'] != 'HOLD'])
                st.metric("Total Signals", total_signals)
                st.metric("Buy Signals", len(df_filtered[df_filtered['signal'] == 'BUY']))
                st.metric("Sell Signals", len(df_filtered[df_filtered['signal'] == 'SELL']))
        ...
        if 'in_sample' in backtest_data and 'out_of_sample' in backtest_data:
            ...
            with col1:
                ...
                for key, value in is_metrics.items():
                    st.metric(label=key.replace('_', ' ').title().replace('%', ''), value=f"{float(value):.2f}{'%' if '%' in key else ''}")
            with col2:
